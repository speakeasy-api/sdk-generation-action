...
var (
	releaseInfoV2Regex   = regexp.MustCompile(`(?s)## (.*?) v(\d+\.\d+\.\d+(?:-\w+(?:\.\w+)*)?)\n.*?Generated by \[Speakeasy\]\(https:\/\/speakeasy\.com\) \[CLI ([^\]]+)\]\(https:\/\/github\.com\/speakeasy-api\/speakeasy\/releases\)`)
...
func ParseReleasesV2(data string) (map[string]releases.LanguageReleaseInfo, string, error) {
	...
	// Validate release format using V2 regex pattern
	releaseMatches := releaseInfoV2Regex.FindStringSubmatch(lastRelease)
	if len(releaseMatches) < 3 {
		return nil, "", fmt.Errorf("error parsing V2 release info: invalid format")
	}
